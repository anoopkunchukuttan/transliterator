...
class TransliterationDecoder: 
    ...
    def _decode_top_n_internal(self,f_input_word,topn=1): 
        ...
        final_top_n_candidates=heapq.nlargest(topn,
                itertools.product([sm_shape[0]-1],xrange(sm_shape[1]),xrange(sm_shape[2])),
                key=lambda ia: score_matrix[ ia[0], ia[1], ia[2] ] )

        nbest_list= [
                        (
                            self._backtrack_nbest_output(max_char_matrix,max_char_matrix,max_nbest_matrix,*candidate),
