...
class UnsupervisedTransliteratorTrainer: 
    ...
    def _get_param_value( self, e_id, f_input_chars ): 
        if f_input_chars in self.f_sym_id_map: 
            ...
            for k in xrange(np.shape(score_matrix)[1]):
                ...
                for m in xrange(np.shape(score_matrix)[1]):
                    v=sum ( map ( log_z ,
                               ...
                               [ score_matrix[j-1,m] , self._bigram_score(m,k) , self._get_param_value(  k , f_input_word[j] )  ] 
                               #[ score_matrix[j-1,m] , self._get_param_value(  k , f_input_word[j] )  ] 
                         ) 
                       )  
                ...
                if j>=2:
                    ...
                    for m in xrange(np.shape(score_matrix)[1]):
                        v=sum ( map ( log_z ,
                                   # prev row score         LM score                    transliteration matrix score
                                   [ score_matrix[j-2,m] , self._bigram_score(m,k) , self._get_param_value(  k ,  f_input_word[j-1] +  f_input_word[j]) ] 
                                   #[ score_matrix[j-2,m] , self._get_param_value(  k ,  f_input_word[j-1] +  f_input_word[j]) ] 
                             ) 
                           )  
                ...
                #print [ j , k , score_matrix[j,k], max_row_matrix[j,k], max_char_matrix[j,k] ]

        #pd.DataFrame(score_matrix).to_html(''.join(f_input_word)+'.html')
...
if __name__=='__main__': 
    ...
    test_fcorpus_fname='1.en'
    test_ecorpus_fname='1.en'
